...
pub fn read(opts: &Opts) -> Result<Config> {
    let (config_path, required) = if let Some(path) = &opts.config {
        ...
        let path = dir.join(env!("CARGO_PKG_NAME")).join("config").with_extension("toml");
    ...
    let config_file = match fs::read_to_string(&config_path) {
        ...
        Err(ref err) if !required && err.kind() == io::ErrorKind::NotFound => {
            log::debug!("Failed to open config file '{}': {}", config_path.display(), err);
            return Ok(Config::default());
        },
        Err(err) => return Err(err.context(format!("failed to open config file '{}'", config_path.display())).into())
    };
